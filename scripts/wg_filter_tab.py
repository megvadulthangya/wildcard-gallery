...
# Blacklist state
blacklist: set = set()
show_blacklist_only: bool = False
        ...
        for tag in config.members:
            ...
            if tag_data:
                ...
                for wild_path in tag_data:
                    ...
                    if wdata:
                        wdata.aux_prompt = config.added_prompt
...
# -------------------------|Blacklist placeholder generation|--------------------

def _build_blacklist_placeholders() -> list[WildcardEntry]:
    """Create minimal placeholder entries for blacklisted wildcards."""
    placeholders = []
    for wpath in blacklist:
        if wpath not in wildcards_dict:
            ...
            entry = WildcardEntry(
                ...
                prompts="",
                aux_prompt="",
                thumbnails={"default": SHARED_ASSESTS["card_fallback"]},
                is_preloaded=True,
                tags=[],
                file_origin="",
                is_locked=False
            ...
            # Should not happen because blacklisted items are excluded from wildcards_dict,
            # but fallback just in case.
            placeholders.append(wildcards_dict[wpath])
    return placeholders


def _rebuild_filtered_pile_from_all():
    """Rebuilds filtered_pile from the current source (normal or blacklist-only)."""
...
def _apply_blacklist_change(paths, add: bool):
    ...
    if paths:
        if add:
            blacklist = blacklist | paths
        else:
            blacklist = blacklist - paths
        save_blacklist(blacklist)

    # Rebuild the main dictionary (skip blacklisted items) and refresh filter module
    if callable(rebuild_callback):
        rebuild_callback(shallow_refresh=False)

    # Re-fetch fresh wildcards_dict and blacklist (rebuild may have changed them)
    global wildcards_dict
    wildcards_dict = rebuild_callback.__self__.cards if hasattr(rebuild_callback, '__self__') else {}
    blacklist = load_blacklist()

    # If we are currently in blacklist-only mode, rebuild the placeholder list
    if show_blacklist_only:
        filtered_pile = _build_blacklist_placeholders()
    else:
        _rebuild_filtered_pile_from_all()
    ...
    if current_page > page_count:
        current_page = page_count
        samples_list = update_gallery_view()
    prefix = "⛔ Blacklist (placeholders)" if show_blacklist_only else "🎴 Filter Results"
    return (
        gr.update(visible=bool(samples_list), samples=samples_list),
...
def act_add_to_blacklist():
    paths = {entry.path for entry in selected_entries}
    return _apply_blacklist_change(paths, add=True)


def act_remove_from_blacklist():
    paths = {entry.path for entry in selected_entries}
    return _apply_blacklist_change(paths, add=False)


def act_toggle_show_blacklist():
    ...
    show_blacklist_only = not show_blacklist_only
    selected_entries = []
    selected_stack_paths = []
    current_page = 1
    filtered_stacks = {}
    ...
    if current_page > page_count:
        ...
        samples_list = update_gallery_view()
    prefix = "⛔ Blacklist (placeholders)" if show_blacklist_only else "🎴 Filter Results"
    label = "Show Blacklist [ON]" if show_blacklist_only else "Show Blacklist"
    return (
        gr.update(value=label),
        gr.update(visible=show_blacklist_only),
        gr.update(value=_blacklist_count_html()),
        gr.update(visible=bool(samples_list), samples=samples_list),
        *gr_update_pages(current_page, page_count),
        gr.update(value=_filter_stat_text(prefix)),
...
def act_blacklist_select_all():
    """Select all currently displayed wildcards (used by blacklist 'Select All' button)."""
    global selected_entries
    global selected_stack_paths
...
def act_run_filter(sel_filter_prop, sel_filter_logic, opt_extend_sel, tx_pos_input, tx_neg_input, sel_pos_input, sel_neg_input):
    ...
    if show_blacklist_only:
        # Blacklist mode: we only show the placeholder entries, but we still allow filtering by path using selector text.
        blacklist_placeholders = _build_blacklist_placeholders()
        if sel_filter_prop == filter_modes[1] and tx_pos_input:
            pos_terms = [term.strip() for term in tx_pos_input.split(",") if term.strip()]
            neg_terms = [term.strip() for term in tx_neg_input.split(",") if term.strip()] if tx_neg_input else []
            for entry in blacklist_placeholders:
                path_lower = entry.path.lower()
                pos_ok = any(term in path_lower for term in pos_terms) if pos_terms else True
                neg_ok = not any(term in path_lower for term in neg_terms) if neg_terms else True
                if pos_ok and neg_ok:
                    filtered_pile.append(entry)
        else:
            # No filter or unsupported filter in blacklist mode – show all blacklisted placeholders
            filtered_pile = blacklist_placeholders
    else:
        # Normal filter (excludes blacklisted items because wildcards_dict already excludes them)
    ...
    if current_stack_level > 0:
        filter_stat_tx += f"  /  ({len(filtered_stacks)}) Packs at stacking lvl [{current_stack_level}]"
    ...
    return (gr.update(visible=bool(samples_list), samples=samples_list),
            ...
            gr.update(visible=True),
            *update_card_mode(is_creation=False, is_edit=False)
            )
...
def act_deselect_all():
    ...
    return (
        gr.update(visible=bool(samples_list), samples=samples_list),
        gr.update(visible=True),
        *update_card_mode(html_view, is_creation=False, is_selection=False, is_edit=False)
    )
...
def enable_creation_mode():
    ...
    return (
        ...
        gr.update(visible=bool(samples_list), samples=samples_list),
        *update_card_mode(creation_pannel_html(), is_creation=True)
    )
...
def act_create_wildcard(tx_edit_wpath, tx_edit_prompt, sel_tag_add, tx_sel_tag_add):
    ...
    new_card = None
    if not tx_edit_wpath:
        gr.Warning("wildcard path is missing")
    elif tx_edit_wpath.endswith('/'):
        gr.Warning("wildcard path must end with name")
    elif not tx_edit_prompt:
        gr.Warning("wildcard prompt is missing")
    else:
        new_card = update_wildcard_yaml(tx_edit_wpath, tx_edit_prompt)
...
def on_ui_tabs():
    with gr.Blocks() as filter_tab:
        ...
        btn_import_wp.click(act_import_wildpack, inputs=[file_browse_wp],
                            outputs=[file_browse_wp, btn_import_wp, html_pack_info, coll_flt_res, btn_create_mode, disp_results, opt_stacks_lvl, *gr_stack_page_selector, *gr_stack_card_editor])

        sel_mgr_tag_groups.select(act_list_tagroup, inputs=[sel_mgr_tag_groups],
        ...
        def toggle_blacklist_row():
            nonlocal blacklist_row_visible
            blacklist_row_visible = not blacklist_row_visible
            return gr.update(visible=blacklist_row_visible)

        btn_blacklist.click(toggle_blacklist_row, outputs=[blacklist_actions_row])

        btn_add_to_blacklist.click(act_add_to_blacklist, outputs=[coll_flt_res, disp_blacklist_count, *gr_stack_card_editor, *gr_stack_page_selector, disp_results])
        btn_remove_from_blacklist.click(act_remove_from_blacklist, outputs=[coll_flt_res, disp_blacklist_count, *gr_stack_card_editor, *gr_stack_page_selector, disp_results])
        btn_show_blacklist.click(act_toggle_show_blacklist, outputs=[btn_show_blacklist, blacklist_actions_row, disp_blacklist_count, coll_flt_res, *gr_stack_page_selector, disp_results])
        btn_blacklist_select_all.click(act_blacklist_select_all, outputs=[coll_flt_res, btn_create_mode, *gr_stack_card_editor])
